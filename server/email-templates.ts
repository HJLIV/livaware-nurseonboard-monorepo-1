...
// ─── 10b. Training course (LMS) reminder ─────────────────────────────
register({
  key: "lms_course_reminder",
  label: "Training course reminder (LMS)",
  description:
    "Reminder sent to a nurse with outstanding/overdue training courses assigned in the LMS. The bullet list of courses is auto-inserted at {{COURSES_LIST}}.",
  category: "nurse",
  envelope: {
    headerTitle: "NurseOnboard",
    headerSubtitle: "Livaware Ltd — Training Course Reminder",
    footerText: "Livaware Ltd — Secure Nurse Onboarding · Training",
    footerSecondLine: AUTOMATED_LINE,
  },
  tokens: [
    { name: "{{NAME}}", description: "Full name" },
    { name: "{{COURSES_LIST}}", description: "Bullet list of outstanding courses (auto-rendered)" },
    { name: "{{COUNT}}", description: "Number of outstanding courses" },
    { name: "{{PORTAL_URL}}", description: "Portal link" },
    { name: "{{EXPIRY}}", description: "Formatted link expiry date" },
  ],
  defaultSubject: "Training courses to complete — action required",
  fields: [
    { name: "greeting", label: "Greeting", kind: "text", default: "Dear {{NAME}}," },
    {
      name: "intro",
      ...
      rows: 3,
      default:
        "You have {{COUNT}} training course(s) assigned that still need to be completed. Please work through them via your secure portal as soon as possible.",
    },
    { name: "boxTitle", label: "Courses box title", kind: "text", default: "Courses to complete" },
    { name: "ctaLabel", label: "Button text", kind: "text", default: "Open my training" },
    {
      name: "expiryNote",
      label: "Expiry / link note",
      kind: "textarea",
      rows: 2,
      default:
        "This link is personal to you — please do not share it. It will expire on {{EXPIRY}}. Your progress is saved as you go.",
    },
    {
      name: "signoff",
      label: "Sign-off",
  ...
  renderBody: (v, t) => {
    ...
    return `
      <p style="font-size:16px; color:#F0ECE4; margin-bottom:8px;">${escapeHtml(applyTokens(v.greeting, t))}</p>
      ${paragraphs(applyTokens(v.intro, t))}
      ${calloutBox(applyTokens(v.boxTitle, t), `<ul style="margin:0; padding-left:20px;">${items}</ul>`)}
      ${ctaButton(applyTokens(v.ctaLabel, t), url)}
      ${fallbackLinkLine(url)}
      ${smallNote(applyTokens(v.expiryNote, t))}
      <p style="font-size:14px; color:#E0DCD4; margin-top:24px; white-space:pre-line;">${escapeHtml(applyTokens(v.signoff, t))}</p>
