...
export async function generateCandidatePDF(candidateId: string): Promise<Buffer> {
  ...
  return new Promise((resolve, reject) => {
    ...
    // Footers are written once at the end via the buffered-pages API so we
    // don't accidentally trigger pdfkit's auto-pagination by writing below
    // the bottom margin during normal content rendering — that previously
    // caused an extra blank page to be inserted every time ensureSpace()
    // started a new page.
    //
    // Writing into the bottom margin would itself trigger pdfkit's auto-page
    // break, so we temporarily relax the bottom margin while writing the
    // footer on each buffered page.
    function writeFootersOnAllPages() {
      ...
      for (let i = range.start; i < range.start + range.count; i++) {
        ...
        doc.save();
        doc.fillColor(LIGHT_GREY).fontSize(7).font("Helvetica").text(
          footerText,
          50,
          doc.page.height - 30,
    ...
    doc.fillColor(LIGHT_GREY).fontSize(7).font("Helvetica-Oblique")
      .text("This report has been generated by Basecamp for Livaware Ltd. All information is subject to verification. " +
