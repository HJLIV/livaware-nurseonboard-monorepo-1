...
- New tables (`shared/schema.ts`): `invoices` (header + bank/personal/totals/status lifecycle), `invoiceTimesheetEntries`, `invoiceAdditionalCosts`. Money stored as pence (int), hours as minutes (int) — no floats. New `invoiceStatusEnum` (`submitted | approved | paid | reconciled | rejected`). Added `invoices` to `auditModuleEnum` and to `super-admin.ts` `VALID_MODULES`.
- Gated to **completed** nurses only via `requireNurseStageCompleted` (returns 403). Status state machine in `server/routes/invoices.ts` `TRANSITIONS`: `submitted → {approved, rejected}`, `approved → {paid, rejected}`, `paid → {reconciled}`, `reconciled` & `rejected` are terminal. Reject requires `rejectedReason`; reconcile requires `paymentReference + paymentDate`. No emails are sent on status change (per spec).
- Server: `server/routes/invoices.ts` exposes:
  - Portal: `GET/POST /api/portal/:token/invoices`, `GET/DELETE /api/portal/:token/invoices/:id` (withdraw only while `submitted`), `POST /api/portal/:token/invoices/:id/attachment` (multer upload of receipt or signed timesheet).
  - Admin: `GET /api/admin/invoices?status=&search=&from=&to=&stage=` (defaults to `stage=completed`, supports `stage=all`), `GET /api/admin/invoices/stats`, `GET /api/admin/invoices/export.csv`, `GET /api/admin/invoices/:id`, `PATCH /api/admin/invoices/:id/status`, `GET/POST /api/admin/invoices/by-nurse/:nurseId`, `DELETE /api/admin/invoices/:id` (super-admin only).
- Audit actions (module `invoices`): `invoice_submitted`, `invoice_withdrawn`, `invoice_approved`, `invoice_paid`, `invoice_reconciled`, `invoice_rejected`, `invoice_deleted`, `invoice_attachment_added`. Source recorded as `portal` or `admin` in `detail.source`.
- Frontend:
  - Portal page `client/src/pages/portal/invoices.tsx` (`/portal/invoices`) — 5-step new-invoice wizard (personal → bank → timesheet entries → additional costs → review), live totals (hours + £), prefill from last invoice, list + detail view, withdraw, attachment upload.
  - Admin matrix `client/src/pages/reports/invoices.tsx` (`/reports/invoices`) — sticky stats by status, filters (status / stage / search / date range), CSV download, click-row side-panel editor with status transition buttons + reject/reconcile dialogs and `indicator-saving` / `indicator-saved` UX from the Availability matrix.
  - Sidebar: `Invoices` item lives under the **People & Invoicing** group (`sidebar-nav.tsx`), alongside Nurses — it's a per-nurse finance surface, not a roster matrix.
  - Portal sidebar: new `Finance > Invoices` group via `invoicesEnabled` + `selectInvoices` (mirrors Availability), surfaced only when `nurse.currentStage === "completed"`.
  - Per-nurse `Invoices` tab in `nurse-detail.tsx` linking to the full report.
- PDF + auto-email on submit: `server/invoice-pdf.ts` renders an A4 PDF via `pdfkit` (navy/gold branding, header, bill-from/bill-to, bank details strip, timesheet table, additional costs, totals box, payment notes, paginated footer). `sendInvoiceSubmittedEmail` in `server/outlook.ts` mails the PDF to `INVOICE_RECIPIENT_EMAIL` (default `invoices@livaware.co.uk`) via Microsoft Graph (`saveToSentItems: true`). Triggered fire-and-forget from `createInvoiceFromSubmission` in `server/routes/invoices.ts` (gated on `isOutlookConfigured()`; failure logs and never blocks the submit response). New audit action `invoice_pdf_emailed` (module `invoices`). Re-download endpoints: `GET /api/portal/:token/invoices/:id/pdf` (own only, completed-stage gated) and `GET /api/admin/invoices/:id/pdf` (admin). FE "PDF" download buttons on portal invoice detail header and admin side panel.
- Tests: `tests/29-invoices.test.ts` (13 tests; T2b verifies portal + admin PDF endpoints stream `application/pdf` with valid `%PDF` header).
